...
public class SparkAligner {
	...
	public static void main(String[] args) throws Exception {
	    ...
	    JavaPairRDD<IntWritable, BytesWritable> refRawSequence = context.sequenceFile(args[0], IntWritable.class, BytesWritable.class);
	    JavaPairRDD<IntWritable, BytesWritable> qryRawSequence = context.sequenceFile(args[1], IntWritable.class, BytesWritable.class);
		...
		System.out.println("PartitionSeesd: " + partitionedSeeds.count() + " partitions:" + partitionedSeeds.partitions().size());
			
		JavaPairRDD<IntWritable, BytesWritable> alignments = partitionedSeeds.mapPartitionsToPair(new PairFlatMapFunction<Iterator<Tuple2<BytesWritable, BytesWritable>>, IntWritable, BytesWritable>() {		
				...
				@Override
				public Iterable<Tuple2<IntWritable, BytesWritable>> call(
						Iterator<Tuple2<BytesWritable, BytesWritable>> iterSeedTuple)
						throws Exception {
					List<Tuple2<IntWritable, BytesWritable>> res = new ArrayList<Tuple2<IntWritable, BytesWritable>>();
					res.add(new Tuple2<IntWritable, BytesWritable>(new IntWritable(2333), new BytesWritable()));
					...
					while (iterSeedTuple.hasNext()) 
					{
						Tuple2<BytesWritable, BytesWritable> record = iterSeedTuple.next();
						merIn = new MerRecord(copyByteFromBytesWritable(record._2));
		...
		System.out.println(alignments.count());
